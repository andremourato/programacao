  ...
  if (key==CODED) {
    ...
    case RIGHT:
      for (int i=0; i<nLin; i++) {
        if (px < centroX(nCol)&&path[positionY(py)][positionX(px)+1]!=0&&py==centroY(i)) {
          vx = vel;
          vy = 0;
          direction = "right";
        }
    ...
    case LEFT:
      for (int i=0; i<nLin; i++) {
        if (px > centroX(1)&&path[positionY(py)][positionX(px)-1]!=0&&py==centroY(i)) {
          vx = -vel;
          vy = 0;
          direction = "left";
        }
    ...
    case DOWN:
      for (int i=0; i<nCol; i++) {
        if (py < centroY(nLin)&&path[positionY(py)+1][positionX(px)]!=0&&py==centroX(i)) {
          vx = 0;
          vy = -vel;
          direction = "down";
        }
    ...
    case UP:
      for (int i=0; i<nCol; i++) {
        if (py > centroY(1)&&path[positionY(py)-1][positionX(px)]!=0&&py==centroX(i)) {
          vx = 0;
          vy = vel;
          direction = "up";
        }
...
public void definePath() {
  ...
  for (int j=0; j<=nLin+1; j++) {
    for (int i=0; i<=nCol+1; i++) {
      if (get((int)centroY(j), (int)centroX(i))!=corObstaculos) path[j][i] = 2;
...
public void detectWall() {
  ...
  //RIGHT
  if (path[positionY(py)][positionX(px)+1]==0&&direction=="right")vx = 0;
  //LEFT
  if (path[positionY(py)][positionX(px)-1]==0&&direction=="left")vx = 0;
  //DOWN
  if (path[positionY(py)+1][positionX(px)]==0&&direction=="down")vy = 0;
  //UP
  if (path[positionY(py)-1][positionX(px)]==0&&direction=="up")vy = 0;
}
